...
from fantasybert.core.trainer_utils import PGD, FGM
from fantasybert.core.trainer_utils import _save_model, _model_contains_inner_module
from fantasybert.core.logger_utils import logger
from fantasybert.core.optimization import ExponentialMovingAverage
...
class Callback(object):
    ...
    def on_batch_begin(self, batch_x, batch_y):
        r"""
    ...
    def on_backward_end(self, batch_x, batch_y):
        r"""
...
class CallbackType(ExplicitEnum):
    SAVEMODEL = "save_model"
    LRSCHEDULER = "lr_scheduler"
    ADVTRAIN = "adv_training"
    ...
    def prepare_callbacks(self, callbacks):
        ...
        if isinstance(callbacks, str):
            ...
            if self.args.save_step_model:
                total_callbacks.append(SaveModelCallback(self.trainer.save_path))
            if self.trainer.scheduler is not None:
                total_callbacks.append(LRSchedulerCallback(self.trainer.scheduler))

            if callbacks == 'default':
                total_callbacks.extend([EarlyStopCallback(self.args.patience)])  # , TensorboardCallback , FitlogCallback()
            else:
                raise TypeError(
                'can only support default and save_model, to use more callback, please pass in Callback module')
        ...
        elif self.trainer.stay_default_callback:
            total_callbacks.extend([EarlyStopCallback(self.args.patience)])
        else:
            raise TypeError(f"Expect callbacks in CallbackManager(callbacks) to be list or str. Got {type(callbacks)}.")
...
class AdversarialTrainingCallback(Callback):
    ...
    def on_backward_end(self, batch_x, batch_y):
        if self.trainer.args.use_adv == 'pgd' or self.trainer.args.use_adv is True:
            ...
            for _t in range(self.pgd_k):
                ...
                prediction = self.trainer._data_forward(self.model, batch_x)
                loss_adv = self.trainer._compute_loss(prediction, batch_y, batch_x).mean()
...
class ExponentialMovingAverageCallback(Callback):
    ...
    def on_step_end(self, batch_x, batch_y):
...
TYPE_TO_CALLBACK_FUNCTION = {
    CallbackType.SAVEMODEL: SaveModelCallback,
    CallbackType.LRSCHEDULER: LRSchedulerCallback,
    CallbackType.ADVTRAIN: AdversarialTrainingCallback,
    CallbackType.GRADIENTCLIP: GradientClipCallback,
    CallbackType.EARLYSTOP: EarlyStopCallback,
    CallbackType.EVALUATE: EvaluateCallback,
    CallbackType.LRFINDER: LRFinderCallback,
    CallbackType.TESTER: _TesterCallback,
    CallbackType.EMA: ExponentialMovingAverageCallback,
    CallbackType.CHECKPOINT: CheckPointCallback,
}
