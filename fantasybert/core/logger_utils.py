...
def _set_stdout_handler(logger, stdout='tqdm', level='INFO'):
    ...
    if stream_handler is not None:
        ...
        # stream_formatter = logging.Formatter(fmt='%(asctime)s - %(module)s - [%(levelname)s] - %(message)s',
        #                   datefmt='%Y/%m/%d %H:%M:%S')
...
class EasyTransformersLogger(logging.getLoggerClass()):
...
logging.setLoggerClass(EasyTransformersLogger)
...
log_path = f"./experiments/logs/run_{time_stamp}.log"
